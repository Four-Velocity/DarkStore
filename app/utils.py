...
def profile_performance_generator(client: finnhub.Client, profile: list[ProfileCurrency]) -> Generator[Decimal, None, None]:
    ...
    for currency in profile:
        ...
        if data['s'] == 'no_data':
            yield 0
            continue
        yield currency.amount * Decimal(data['c'][-1]) - currency.amount * Decimal(data['c'][0])


def get_currency_values(client: finnhub.Client, currency: str, ytd_borders: tuple[int, int]) -> Iterable[Decimal]:
    data = client.crypto_candles(f"BINANCE:{currency}USDT", "D", *ytd_borders)
    if data['s'] == 'no_data':
        return [Decimal(0)]
...
def get_profile_min_max(values: Iterable[Iterable[Decimal]]) -> tuple[tuple[Decimal, int], tuple[Decimal, int]]:
    min_value = None
    max_value = None
